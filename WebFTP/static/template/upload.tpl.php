		...
		if ($_FILES["Filedata"]["error"] > 0){
			...
			if(file_exists($uploadfile) && !(bool)$_REQUEST['cover']){
			    ...
			    if(move_uploaded_file($_FILES["Filedata"]["tmp_name"], $uploadfile)){
					echo $file['msg_success_normal'],$file['info'];
					exit();
				}else{
					echo $file['msg_attention'],$file['info'];
					exit($file['msg_error_unknow']);
				}
			}
		}
	}else{
...
.attention
{
	background: #fffbcc url('static/images/ico/exclamation.png') 10px 11px no-repeat;
	border-color: #e6db55;
	color: #666452;
}
.information
{
	background: #dbe3ff url('static/images/ico/information.png');
	border-color: #a2b4ee;
	color: #585b66;
}
.success
{
	background: #d5ffce url('static/images/ico/tick_circle.png');
	border-color: #9adf8f;
	color: #556652;
}
.error
{
	background: #ffcece url('static/images/ico/cross_circle.png');
	border-color: #df8f8f;
	color: #665252;
}
.notification div
...
function dealsize(size){
  ...
  function challs_flash_update(){ //Flash 初始化函数
  	...
  	a.parameter="action=upload&type=update&path=<?php echo urlencode($_REQUEST['path']);?>";
  	//设置提交参数，以GET形式提交

  	a.typefile=["All File (*.*)","*.*;", "Web File(*.html,*.htm,*.zip, ...)","*.html;*.htm;*.php;*.asp;*.js;*.css;*.gif;*.png;*.jpg;*.jpeg;*.zip;","Script File(*.php,*.asp,*.js,*css)","*.php;*.asp;*.js;*.css;","Images File(*.gif,*.png,*.jpg,*jpeg)","*.gif;*.png;*.jpg;*.jpeg;"];
  	//设置可以上传文件 数组类型
...
<form name="Filedata">
  ...
  <input name="type" id="type" type="hidden" value="update">
</form>
